...
- **RSVP is built**: intro + `.rsvp-deadline` ("Please reply by 31st March 2027") + a big green `.rsvp-button`. Approach = **Google Form, button link-out** (opens in new tab). **The button `href` is still a placeholder `#`** — swap it for the real Google Form link (and add `target="_blank" rel="noopener"`) once the user creates the form. There's a `TODO` comment on it in `index.html`. I made a form header banner from the save-the-date at `scratchpad/rsvp-header.png` (top strip: lights + "Save the Date" title) for the user to upload to Google Forms.
- **All five sections are now built.** Remaining: drop in the real RSVP form link.
